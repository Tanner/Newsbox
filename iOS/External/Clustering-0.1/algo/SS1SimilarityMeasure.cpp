/*
** SS1SimilarityMeasure.cpp
** Login : Julien Lemoine <speedblue@happycoders.org>
** Started on  Sun Sep 17 19:07:26 2006 Julien Lemoine
** $Id$
** 
...
Algo::SS1SimilarityMeasure::SS1SimilarityMeasure(const std::vector<unsigned> &docCards,
						   unsigned nbDescriptors) :
  Algo::SimilarityMeasure(docCards, nbDescriptors)
{
}

Algo::SS1SimilarityMeasure::~SS1SimilarityMeasure()
{
}

double Algo::SS1SimilarityMeasure::compute(const Cluster &c1, const Cluster &c2) const
{ 
  ...
  denum = sizeC1 * sumC2 + sizeC2 * sumC1;
  num = sizeC1 * sizeC2 * _getNbDescriptors();
  num -= denum;
  num += 2.0 * dotProduct;
  num *= 2.0;
  denum += num - 2.0 * dotProduct;
  return num / denum;
}

double Algo::SS1SimilarityMeasure::compute(unsigned docId1, unsigned docId2, unsigned inter) const
{
  double num = _getNbDescriptors() + 2.0 * inter - _getDocCard(docId1) - _getDocCard(docId2);
  num *= 2.0;
  double denum = num + _getDocCard(docId1) + _getDocCard(docId2) - 2.0 * inter;
  return num / denum;
}
