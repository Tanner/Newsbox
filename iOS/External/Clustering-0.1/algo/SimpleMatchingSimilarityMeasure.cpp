/*
** SimpleMatchingSimilarityMeasure.cpp
** Login : Julien Lemoine <speedblue@happycoders.org>
** Started on  Sun Sep 17 18:49:23 2006 Julien Lemoine
** $Id$
** 
...
Algo::SimpleMatchingSimilarityMeasure::SimpleMatchingSimilarityMeasure(const std::vector<unsigned> &docCards,
						   unsigned nbDescriptors) :
  Algo::SimilarityMeasure(docCards, nbDescriptors)
{
}

Algo::SimpleMatchingSimilarityMeasure::~SimpleMatchingSimilarityMeasure()
{
}

double Algo::SimpleMatchingSimilarityMeasure::compute(const Cluster &c1, const Cluster &c2) const
{ 
  ...
  double dotProduct = 0, num = 0, denum = 0;
  ...
  denum = sizeC1 * sizeC2 * _getNbDescriptors();
  num = denum;
  num -= sizeC1 * sumC2 + sizeC2 * sumC1;
  num += 2.0 * dotProduct;
  return num / denum;
}

double Algo::SimpleMatchingSimilarityMeasure::compute(unsigned docId1, unsigned docId2, unsigned inter) const
{
  double num = _getNbDescriptors() + 2.0 * inter - _getDocCard(docId1) - _getDocCard(docId2);
  double denum = _getNbDescriptors();
  return num / denum;
}
